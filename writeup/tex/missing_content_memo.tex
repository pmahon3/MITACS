...
(ii) stratified Patra--Sen as a $\sigma$-algebra adequacy
diagnostic; (iii) synthetic Gaussian-DGP calibration to anchor
the residual-$\chi^{2}$ floor against a known reference.
...
stratified Patra--Sen, synthetic Gaussian-DGP calibration). \\
\addlinespace
...
Thread \newline (kernel-family)
  ...
  & \texttt{pending\_amendment} after Q2B (R-C3 has no entry in \texttt{branching\_rules}); substantive recommendation is \texttt{exhausted} on the pre-registered distributional-class scope per (S9)~$+$~(S10), with a fresh thread on the framework's three resolution paths as the natural next move \\
\bottomrule
\end{tabular}
...
\path{~/Research/Mathematics/Resolvent_Framework/notes/unsorted/disintegration_diagnostic.md}.
\end{itemize}

\paragraph{Next concrete step.}
A thread move via \verb|thread-coordinator| advancing the
\texttt{distributional-class-thread} from \texttt{pending\_amendment}.
Under the framework reframe (S9~$+$~S10 jointly bound the two
distributional-class levers), the substantive recommendation is
to declare the thread \texttt{exhausted} on its pre-registered
scope and spawn a fresh thread on the framework's three
resolution paths: (i) candidate-$Z$ testing
(temperature, secular drift via \texttt{year\_since\_cutoff} per
(S5), refined day-type partition); (ii) stratified Patra--Sen as
a $\sigma$-algebra adequacy diagnostic; (iii) synthetic
Gaussian-DGP calibration to anchor the residual-$\chi^{2}$ floor.
A mechanism-redesign amendment to Q2B (e.g., two-sided variance
rescaling addressing the under-dispersion direction; weather-
conditioned mean-bias) is the other workflow-legitimate move, but
remains within the bounded levers (S9)~$+$~(S10) characterised.
