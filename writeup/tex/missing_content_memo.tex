...
of family. Bounded by (S9): the
reduction curve M0 $\to$ M1 ($16\times$) versus M1 $\to$ richer-best
($1.44\times$) shows the distributional-class lift is bounded;
further reduction of the marginal pathology requires non-distributional
content. \\
\addlinespace
...
process
& \textbf{Empirically bounded} by (S9). Three richer-than-Student-$t$
families tested (mixture-2-Gaussian M3, mixture-3-Gaussian M4, KDE
residual law M5). All three are $\chi^{2}$-indistinguishable (spread
$13$ $\chi^{2}$ units across the three; KDE is the \emph{worst} by
$0.8$). The reduction curve M0 (Gaussian) $\to$ M1 (Student-$t$) is
$16\times$ ($22{,}329 \to 1{,}371$); M1 $\to$ Q2A-best is $1.44\times$
($1{,}371 \to 954$). Picture (C) is not refuted (the defect remains;
$\mathrm{D\_RATIO}_{\Studt} > 1$ with CI clear of $1$ per (S8c))
but distributional-class flexibility above a $2$-component symmetric
mixture has near-zero marginal return on this population. The
remaining $\sim 954$ $\chi^{2}$ is non-distributional; the
day-type asymmetry (saturday $\sim 4\times$ weekday, family-invariant)
corroborates (S2)/(S3) and points toward state-dependent or
exogenous content. \\
\addlinespace
...
Thread \newline (kernel-family)
  ...
  & \texttt{pending\_amendment} after Q2A; analyst chooses among reopen-Q2B, new Q2C (state-dependent / hybrid), or spawn non-distributional thread (see (S9) and Q2A arbiter) \\
\bottomrule
\end{tabular}
...
A thread amendment via \verb|thread-coordinator| advancing the
\texttt{distributional-class-thread} from \texttt{pending\_amendment}.
The (S9c) asymptote reading favours either reopening Q2B
(non-distributional content: iterated-mean bias, day-anchor seam,
secular drift) or spawning a fresh non-distributional thread on
the remaining $\sim 954$ $\chi^{2}$. State-dependent or hybrid
distributional/calendar models (a possible Q2C node) are a
middle option; the Q2A arbiter does not rank them.
