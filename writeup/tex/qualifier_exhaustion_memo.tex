...
artefact (retraction).} A first reading of this experiment
identified a prominent \(h = 1\) channel: one day
...
nats), and that one day carried the pooled \(h = 1\) mean
\(\Delta\mathrm{LL}\) of \(+0.12\) under the Gaussian \(\to\) Student-\(t\)
swap. Section~\ref{sec:option-k-seam-audit} below documents that
this was driven by a separable pipeline issue (month-boundary
climatology lookup discontinuity), not by an intrinsic heavy-tail
property of the conditional law. The \(h = 1\) signal does not
survive seam exclusion: pooled \(h = 1\) \(\Delta\mathrm{LL}\) drops
from \(+0.12\) to \(-0.02\) with CI straddling zero. The narrative
``Student-\(t\) provides insurance against rare \(7\sigma\) events
at \(h = 1\)'' is therefore \emph{retracted}; what Student-\(t\)
provides is a modest mid-horizon improvement (Section~\ref{sec:option-k-seam-audit}).
