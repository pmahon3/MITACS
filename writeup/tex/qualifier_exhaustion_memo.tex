...
artefact (retraction).} This section initially claimed a prominent
\(h = 1\) catastrophic-event channel: one day
...
day-bootstrap CIs excluding zero in both cases (all-days CI
\([0.007, 0.041]\); excl-first CI \([0.005, 0.031]\)). What Student-\(t\) actually
...
conditional law and that the empirical Ontario landings on those
levers were marginal or null. A natural follow-up question: how much
of the per-anchor library-residual mass is genuinely non-Gaussian
mixture mass, distinct from the predictive-Gaussian null
\(\mathcal{N}(0,1)\)? Patra \& Sen
(2016)~\cite{patra_sen_2016}\footnote{Patra, R.\ K., and B.\ Sen,
...
Series B} 78(4):869--893 (2016); arXiv:1204.5488.} give a published
consistent estimator
\(\hat\alpha_0\) of the mixing proportion in
\(F = \alpha F_s + (1-\alpha) F_b\) with \(F_b\) fully specified, and
a distribution-free finite-sample lower confidence bound
\(\hat\alpha_L^{95\%}\) (their Theorem~5) satisfying
...
the true mixing fraction: the headline numbers therefore
understate the true contamination fraction.
...
anchor; pool across anchors to give \(n_{\mathrm{pooled}} = 60{,}000\)
standardised residuals per cell. Apply Patra--Sen with
\(F_b = \Phi\) (standard normal CDF) at the published tuning
(\(c_n = 0.1 \log\log n\) for the point estimate; \(c_n = 0.6792\)
for the \(95\%\) lower CI, Anderson--Darling
quantile).

\paragraph{Implementation provenance.} A Python implementation of
the PAVA-based estimator (\texttt{scratch/patra\_sen.py}) was
verified against the paper's Figure~1 synthetic settings
...
\end{tabular}
\end{center}
The lower 95\% confidence bounds span \(0.49\) (Mon-hour weekday) to
\(0.74\) (Mon-hour and Fourier sunday) across the six conditions.
...
mixing fraction \(\alpha_0\).

\paragraph{Pipeline-aware null.} The same protocol on synthetic VAR(1)
with Gaussian innovations through the identical production estimator
(\texttt{processing/innovations/validation/synthetic}) gives
\(\hat\alpha_L^{95\%} = 0.002\): the local-WLS-fit-and-pool protocol
does not produce false positives.

\paragraph{No single heavy-tailed law explains the result.} Two
discriminating checks rule out single-law explanations:
\begin{itemize}[leftmargin=2em]
  \item AR(1) with Student-\(t(\nu)\) innovations at population-kurt-
  matched \(\nu \in \{4.27, 4.10, 4.05, 4.02, 4.01\}\) gives
  \(\hat\alpha_L^{95\%} \in [0.20, 0.24]\) (single seed); at
  ...
  \item Ontario \(\hat\alpha_L^{95\%}\) (\(0.49\)--\(0.74\)) is
  \(2.0\)--\(3.5\times\) the maximum any single AR(1)+\(t\) configuration
  produces. The Ontario residuals are not a single heavy-tailed
  AR(1); they have mixture structure beyond what any pure
  heavy-tailed law generates.
\end{itemize}

\paragraph{Anchor-seed stability across all six cells.} Five anchor
RNG seeds per cell: all six \(\hat\alpha_L^{95\%}\) ranges have
spread \(< 0.04\) (table above, last column). The headline
\(0.49\)--\(0.74\) range is defensible across the full result
table, not just a representative cell.
...
negative empirical lever-comparison findings
(\textsf{K}, \textsf{S}, \textsf{S+K}, \textsf{H} all marginal or
null under clean preprocessing), this reframes the practical
conclusion: \emph{the contamination is large in mass but
structurally orthogonal to the lagged-\(z\) state} our levers
...
licensed direction for accommodating the heavy-tailed conditional
law.

\paragraph{Identifiability caveat (the right direction).} Patra--Sen
Lemma~4: for contamination sharing support with \(F_b\),
\(\hat\alpha_0\) estimates \(\alpha_0 = \alpha \cdot
\{1 - \operatorname{ess inf}(f_s/f_b)\} \leq \alpha\). The headline
\(0.49\)--\(0.74\) therefore \emph{underestimates} the true mixing
fraction. The honest framing: ``Patra--Sen's \(\alpha_0\)
(a lower bound on the true mixing fraction) is \(0.49\)--\(0.74\)
in Ontario.''

\paragraph{Preprocessing comparison.} Fourier preprocessing reduces
pooled kurt substantially (40\(\to\)22 weekday, 28\(\to\)20 saturday)
but \(\hat\alpha_L^{95\%}\) moves much less (Mon-hour weekday 0.49
\(\to\) Fourier weekday 0.51; Mon-hour saturday 0.64 \(\to\) Fourier
saturday 0.52; sunday unchanged at 0.74). This is consistent with
the seam-audit finding: the seam affected the \emph{tail shape}
(kurt) but the underlying mixture \emph{structure} is
preprocessing-independent. The contamination is intrinsic to
\(\kappa_Q\), not introduced by preprocessing.

\section{Integration decision}
\label{sec:integration}

With all four design options' empirical landings and the
Patra--Sen contamination quantification in hand, the verdict shape
is clear at three levels:

\emph{Framework}: the \(\kappa_Q\) programme \emph{permits}
kernel-form, state-augmentation, and per-horizon relaxations;
\(\kappa_Q\)'s law-agnostic content supports all four design
options (\textsf{K}, \textsf{S}, \textsf{H}, \textsf{S+K}).
Rebaseline-grade evidence (per-anchor library kurt \(24\)--\(33\))
of heavy-tailedness motivates the search.

\emph{Empirical magnitude on the contamination}: at least
\(49\)--\(74\%\) of per-anchor library mass is non-Gaussian
mixture mass (Patra--Sen \(\hat\alpha_L^{95\%}\), conservative
lower bound by Lemma~4). This is large and unambiguous; the
...
(Fourier) preprocessing, none of \textsf{K}, \textsf{S}, \textsf{S+K},
\textsf{H} delivers a statistically clean forecast-accuracy
improvement on routine-day data. Combined with the contamination
quantification, the only consistent reading is that \emph{the
contamination is structurally orthogonal to the lagged-\(z\) state}
on which all four levers operate. Exogenous information
...
permitting the three classes of lever; the empirical comparison on
the \(n = 132\) dev set finds them all marginal-to-null under clean
preprocessing; the Patra--Sen contamination quantification reframes
the negative empirical findings as evidence that the licensed
direction is exogenous information rather than \(z\)-state
augmentation; the month-boundary seam is documented as a separable
preprocessing limitation and Fourier preprocessing as the cleaner
choice. The sibling-memo arrangement (this document) is the right
place for the detailed exhaustion analysis; the application
paper's \S4.3 inherits the verdict statement above.
