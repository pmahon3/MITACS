...
a registered forward experiment, and an explicit accounting of what
can and cannot be compared against IESO's own published forecasts.

The estimator targets the conditional-regularity kernel \(\kQ\)
determined by the data measure via Rokhlin disintegration; the
correspondence is stated in Section~\ref{sec:method} along with the
three scope qualifiers that bound it.
...
the predictive law is carried entirely by \(\Sj\) and its departure
...
non-Dirac regime (\(\Sj \neq 0\)). The qualifier ``locally'' is
deliberately omitted: per Section~\ref{sec:construction} no
statistically meaningful locality scale is recoverable at this
embedding, and the drift reduces to a single global linear operator.
...
neither is established at all. Document both, engineer neither.

\begin{table}[h]
\centering
\caption{Error-decomposition verdict on the 2021--22 development-set
benchmark. \(\DA\), \(\DB\) are the relative recoverable losses
defined in Eq.~\ref{eq:DA-DB}; brackets give 95\% day-bootstrap CIs.
\(\nuhat\) is the Student-\(t\) shape fitted on the residuals. The
materiality threshold is \(\taumin = 0.01\).}
\label{tab:verdict}
...
Population
  ...
  & Verdict \\
\midrule
Full delivery-day population
  & \(0.048\;[0.007,\,0.086]\)
  & \(0.023\;[-0.000,\,0.048]\)
  & \(6.5\)
  & neither clears \(\taumin\) \\
Single-dimension subpopulation
  & \(\approx 0.061\)
  & \(\approx 0.091\)
  & ---
  & both clear \(\taumin\); CIs overlap \\
\bottomrule
\end{tabular}
\end{table}
...
artefact, not solely a property of the innovation law. Propagation is
a fixed-maximum-dimension state augmentation carrying unused lag
...
Three gates call the same estimator code used elsewhere in the paper
(not reimplementations); their structure is summarised in
Table~\ref{tab:gates}. All three pass.

\begin{table}[h]
\centering
\caption{Synthetic-ground-truth validation gates. Each calls the same
estimator code used in the main pipeline (not a reimplementation),
which is what makes its pass a check on the estimator itself.}
\label{tab:gates}
\begin{tabular}{l l l}
\toprule
Gate & System tested & Check \\
\midrule
Recovery
  & VAR(1) with known \((A, Q)\)
  & drift and diffusion recovered within tolerance \\
Non-Gaussianity
  & Gaussian- vs.\ Student-\(t\)-innovation VAR(1)
  & Gaussian reads \(\approx\)Gaussian; Student-\(t\) flagged heavy-tailed \\
Multi-step composition
  & Iterated \(\Pidelta\) vs.\ VAR(1) closed form
...
seasonal-persistence baseline. Rows are to be filled with the
