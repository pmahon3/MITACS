...
\begin{enumerate}
  ...
  reconciliation).
\end{enumerate}
\todo[inline]{End-to-end within-harness comparison: both pipelines
run through the same dev-window diagnostic at per-day-type
elbow-selected dimensions on their respective series (seasonal-spec
on \(z\), no-transform-spec on raw \(D\)). Four-channel result:
(1) \(\MAE_{\mathrm{seasonal}} = [\ldots]\) MW vs.\
\(\MAE_{\mathrm{raw}} = [\ldots]\) MW (\(\MAPE\) likewise);
(2) fraction-finite-at-24-horizons: seasonal \([\ldots]\), raw
\([\ldots]\);
(3) demand-space \(95\%\) interval coverage, per-hour worst-gap
\([\ldots]\) pp and average-gap \([\ldots]\) pp;
(4) propagation-aware \(\nuhat\) seasonal \([\ldots]\) vs.\
raw \([\ldots]\).
Removal verdict per the pre-registered all-channel rule:
\([\text{licensed} / \text{hold}]\). The production-frozen reference
of \(\approx 786\) MW / \(4.78\%\) reported in
