...
\begin{equation}
\label{eq:results}
\DA \;\approx\; 0.048 \;\;[0.007,\,0.086], \qquad
\DB \;\approx\; 0.023 \;\;[-0.000,\,0.048], \qquad
\nuhat \;\approx\; 6.5,
\end{equation}
(95\% day-bootstrap CIs in brackets). \textbf{Neither lower bound
clears \(\taumin\), and the \(\DB\) interval includes zero.} The
verdict is the catch-all ``neither dominates'' --- not because the
effects are real and tied (the single-dimension subpopulation case,
where both cleared \(\taumin\) at \(\DA \approx 0.061\) and
\(\DB \approx 0.091\)) but because on the complete,
correctly-propagated data neither is established at all.
Document both, engineer neither.
...
\todo[inline]{Per-horizon table from the recorded backtest result.}
