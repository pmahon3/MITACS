...
\begin{enumerate}
  ...
  \item \textbf{Diffusion conditioning.} Compare per-day-type
  condition number \(\kappa(\Sj)\). The rebaseline established that
  intra-day \(\kappa(\Sj) \in [6,\, 1072]\) across day-types in the
  seasonal pipeline; full-process scales were catastrophically
  ill-conditioned (up to \(\sim 10^{20}\)). Tolerance:
  \(\max_{\mathrm{dt}} \kappa_{\mathrm{raw}}(\Sj) /
  \kappa_{\mathrm{seasonal}}(\Sj) < 10\) (an
  order-of-magnitude band; conditioning is log-scale, and the
  relevant contrast is usable vs.\ unusable).
  \item \textbf{Interval coverage.} Compare empirical coverage of
  the nominal \(95\%\) Gaussian-proxy intervals on the same dev
  window. The seasonal pipeline's intervals are calibrated in
  z-space and re-multiplied by \(\sigma_{m,h}\) in demand-space; the
  raw-demand pipeline has no scale step and would deliver
  uniformly-scaled intervals. Tolerance: \(|\text{coverage}_{\mathrm{raw}}
  - \text{coverage}_{\mathrm{seasonal}}| < 2\) percentage points
  (comparing two miscalibrations to each other, not either to
  nominal --- Section~\ref{sec:caveat}'s Gaussian-proxy caveat
...
elbow-selected dimensions on raw demand, four-channel comparison
populated as: (1) \(\MAE = [\ldots]\) MW, \(\MAPE = [\ldots]\)\%
(vs.\ \(\approx 786\) MW / \(4.78\%\)); (2) per-day-type
\(\kappa(\Sj) = [\ldots]\) (vs.\ \([6, 1072]\)); (3) empirical
\(95\%\) interval coverage \([\ldots]\) (vs.\ seasonal \([\ldots]\));
(4) propagation-aware \(\nuhat = [\ldots]\) (vs.\ \(6.5\)).
Removal verdict per the pre-registered all-channel rule:
\([\text{licensed} / \text{hold}]\).}
