...
- `writeup/tex/missing_content_memo.tex` → `missing_content_memo.pdf` — the
  **kernel-family-agnostic theory memo**, 16pp, builds clean. Master citation
  of (S1)–(S8) settled findings. Added 2026-05-26 from the seed
  `notes/seeds/multiscale_factor_coherence.md`. NOT a publication target;
  internal memo that documents the framework + Q1 result.

### Settled findings since 2026-05-21 (chronological)
The applied-audit workflow has produced five settled / arbitered findings.
Authoritative source is the memo's (S1)–(S8) numbering + the per-experiment
memory files.
...
- `2026-05-26_missing-content-thread`: **exhausted** under registered
  ...
  requires amendment).
- `2026-05-26_distributional-class-thread`: **active**, current node Q2D
  by mechanical advancement from Q1's R-D verdict. The Q1 arbiter
  flagged this as pending amendment — the right next move is a
  thread amendment via `thread-coordinator` to revise the R-D
  criterion (it conflated "estimator broken" with "data approximately
  Gaussian at this aggregation level"). After amendment, Q1's
...
- Q2 of `distributional-class-thread` (after R-D-criterion amendment).
