"""Sample-size extension of scratch/k_capture.py. Same forward
iteration + per-step capture; only the dev window changes. The
extension probes whether n=132's wide CIs (advisor note) tighten
with more days.

Default: WIN_END = 2022-10-18 (n~365). Override via CLI:
  python -m scratch.k_capture_extended 2023-10-18    # n~730
  python -m scratch.k_capture_extended 2026-05-17    # n~1540

Output: /tmp/k_step_sequences_extended.pkl

Climatology drift caveat: per-year mean z drifts +0.20 z-units/year
post-cutoff (raw z), but the existing 132-day h=1 RESIDUAL drift is
~-0.02 z-units/year (the forecast absorbs the trend via the lag
state). Phase 1 of the extension verifies whether this residual
stationarity holds on extended data; if not, a linear-residual-
detrend is added to scoring (not capture).
...
from scratch.benchmark_2021_22_ieso import (
    ANCHOR_H, CUTOFF, WIN_START, _refrozen_dims,
)
from scratch.multistep_variance_propagation import augmented_state_transition

# CLI override
if len(sys.argv) > 1:
    WIN_END = pd.Timestamp(sys.argv[1])
else:
    WIN_END = pd.Timestamp("2022-10-18")

OUT_PATH = Path("/tmp/k_step_sequences_extended.pkl")
...
def main() -> None:
    t0 = time.time()
    _say(f"WIN_END = {WIN_END.date()}", t0)
    ...
    _say(f"window: {WIN_START.date()} to {WIN_END.date()}", t0)
    ...
    with OUT_PATH.open("wb") as f:
        pickle.dump({"captures": captures, "D_MAX": int(D_MAX),
                     "dims": dims, "anchor_h": int(ANCHOR_H),
