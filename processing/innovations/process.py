...
from interface_processing import save_all   # <— our updated interface
...
def compute_vector_residuals(anchor_time: pd.Timestamp) -> tuple[pd.Timestamp, np.ndarray, np.ndarray]:
    ...
    for i, θ in enumerate(theta_grid_global):
        proj.kernel.theta = float(θ)
        ...
        try:
            coef_mat = wls_res.coefficients  # shape = (d, d) or Tensor
            if isinstance(coef_mat, torch.Tensor):
                coef_mat = coef_mat.cpu().numpy()
        except Exception:
            coef_mat = np.full((d, d), np.nan, dtype=float)

        # Store coefficient (if shape matches)
        if coef_mat.shape == (d, d):
            C[i, :, :] = coef_mat

        # Extract prediction vector (if present)
        pred_df = wls_res.predictions
        if not pred_df.empty:
            # The index is a MultiIndex (anchor_time, anchor_time+freq)
            try:
                pred_vec = pred_df.loc[(anchor_time, next_time)].values  # shape = (d,)
                R[i, :] = true_vec - pred_vec
            except KeyError:
                # If the (t, t+1) key is missing, leave R[i,:] as NaNs
                pass
