...
for daytype in daytypes:
    ...
    for ax, daytype in zip(axes, daytypes):
        # 1) load & clean
        data = pd.read_csv(f'./csv/results_{daytype}.csv', index_col=0)
        data = data[data < 5].dropna()
        data.index = pd.to_datetime(data.index)
        ...
        # 2) boxplot: one box per column (parameter 0–10)
        data.boxplot(
            ax=ax,
            showfliers=False,  # hide extreme outliers if you like
            grid=False
        )
        ax.set_title(daytype.capitalize())
        ax.set_xlabel("Parameter value")
        if ax is axes[0]:
            ax.set_ylabel("Error")

    plt.savefig(f"./plots/mape_boxplots.png", dpi=300)
