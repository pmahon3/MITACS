...
tree = etree.ElementTree(file="demand_zones.kml")
...
for parent in tree.findall("//%sPolygon/.." % xmlns):
    ...
    df_points[polygon_name] = polygon.contains_points(
        df_points[["longitude", "latitude"]]
    )

# Dump to CSV
df_points.to_csv("weather_station_zones.csv", index=False)
