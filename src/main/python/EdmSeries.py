...
class EdmSeries:
    ...
    def embed_plotting(self, plot_out_dir):
        ...
        for i in range(self.maxTp - self.minTp):
            plt.plot(avg[i]['E'], avg[i]['rho'], color='red',
                     alpha=(self.maxTp - self.minTp - i) / (self.maxTp - self.minTp))
        ...
        # compute rho vs. tp across various embeddings (E, tau)
        for i in range(self.maxE - self.minE):
            print("\n" + str(i), end=": ")
            for j in range(self.maxTp - self.minTp):
                print(j, end=" ")
                tp_curves[i][j] = avg[j].iloc[i]['rho']
            plt.plot(range(self.minTp, self.maxTp), tp_curves[i], color='blue',
                     alpha=(self.maxE - self.minE - i) / (self.maxE - self.minE))
        plt.xlabel('Tp')
        plt.ylabel(r'$\rho$')
    ...
    def smaps_plotting(self, plot_out_dir):
        ...
        for i in range(self.maxE - self.minE):
            ...
            for j in range(self.maxTp - self.minTp):
                ...
                plt.ylabel(r'$\rho$')
            avg_['rho'] = avg_['rho'] / (self.maxTp - self.minTp)
            avg[i] = avg_
        ...
        for i in range(self.maxE - self.minE):
            plt.plot(avg[i]['Theta'], avg[i]['rho'], color='red',
                     alpha=(self.maxE - self.minE - i) / (self.maxE - self.minE))
