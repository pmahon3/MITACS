...
step_size = 1
target = 'Load (kW)'
variables = ['Load (kW)', 'Pressure_kpa', 'Cloud Cover (%)', 'Humidity (%)', 'Wind Speed (kmh)']
method = 'smap'
...
# In Directories
data_file = r'C:\Users\Patrick\OneDrive - The University of Western Ontario\Documents\Research\MITACS\Code\src\main\resources\data\IEEE\data_combined\actuals.csv'
...
library_times = data.loc[pd.Timestamp('2020-09-01'):pd.Timestamp('2020-12-07 07:00:00')].index
training_times = data.loc[pd.Timestamp('2020-12-07 07:00:00'):pd.Timestamp('2020-12-21 07:00:00')].index
prediction_times = data.loc[pd.Timestamp('2020-12-21 07:00:00'):pd.Timestamp('2021-01-04 07:00:00')].index
...
for variable in variables:
    print(variable + ':')
    dimensionalities[variable] = dimensionality(embedding=embedding,
                                                projector_=smap(),
                                                target=variable,
    ...
    optimal_dimensions[variable] = dimensionality_[dimensionality_['rho'] == dimensionality_['rho'].max()].index[0]
print()
...
for variable in variables:
    ...
    opt_Theta = nonlinearity_[nonlinearity_['rho'] == nonlinearity_['rho'].max()].index[0]
