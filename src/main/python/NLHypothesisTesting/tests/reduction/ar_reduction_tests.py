...
split = 0.8
start_idx = int(split*steps+1)
prediction_start = int(split * steps)

# Computing

lorenz_x, lorenz_y, lorenz_z = lorenz(steps=steps,
                                      step_size=step_size,
                                      noise_standard_deviation=noise_standard_deviation)

hypothesis = NonlinearHypothesis(lorenz_x)

reduction = ARReduction(lorenz_x)
...
plt.xlabel('Time')
plt.ylabel('x')
plt.title('x Values for Canonical Lorenz\nSteps: ' + str(steps) + ', Step Size: ' + str(step_size) +
          ', Measurement Error Stdev: ' + str(noise_standard_deviation))
plt.show()
...
plt.plot(reduction.model_array[start_idx:, 0], label='Lorenz x')
