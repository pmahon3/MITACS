...
def grouped_avg(array, n=2):
    result = np.cumsum(array, 0)[n - 1::n] / float(n)
    result[1:] = result[1:] - result[:-1]
    return result


residuals = data - predictions
rdm = np.stack((data[0:-1], residuals[1:]), axis=-1)
rdm = rdm[np.argsort(rdm[:, 0])]
rdm = grouped_avg(rdm, n=10)
plt.scatter(rdm[:, 0], rdm[:, 1])
plt.xlabel('$\mathregular{x_t}$')
plt.ylabel('$\mathregular{R_{t+1}}$')
plt.show()
